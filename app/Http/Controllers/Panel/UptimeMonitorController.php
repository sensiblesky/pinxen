...
{
    /**
     * Display a listing of all uptime monitors (admin view - shows all users' monitors).
     */
    public function index(Request $request): View
    {
        // Start building query
        $query = UptimeMonitor::with(['user', 'checks' => function($query) {
        ...
        if ($request->filled('url')) {
            $query->where(function($q) use ($request) {
                $q->where('url', 'like', '%' . $request->input('url') . '%')
                  ->orWhereRaw('LOWER(url) LIKE ?', ['%' . strtolower($request->input('url')) . '%']);
            });
        }
        ...
        foreach ($monitors as $monitor) {
            ...
            $ssl = null;
            if ($domain) {
                // Check across all users for admin view
                $whois = DomainMonitor::whereRaw('LOWER(TRIM(domain)) = ?', [$domain])
                    ->first();

                $ssl = SSLMonitor::whereRaw('LOWER(TRIM(domain)) = ?', [$domain])
                    ->first();
            }
        ...
        return view('panel.uptime-monitors.index', [
            'monitors' => $monitors,
            'monitorMeta' => $monitorMeta,
            'filters' => $request->only(['name', 'url', 'status', 'created_from', 'created_to']),
        ]);
    }

    /**
     * Search users for Select2 AJAX (used in filter form).
     */
    public function searchUsers(Request $request): \Illuminate\Http\JsonResponse
    {
        $search = $request->input('q', '');
        
        $users = \App\Models\User::where('role', 2)
    ...
    public function create(): View
    {
        ...
        return view('panel.uptime-monitors.create', [
            'users' => $users,
        ]);
    }
    ...
    {
        // Admin can create monitors for any user
        ...
        $validated = $request->validate([
            ...
            'url' => [
                ...
                function ($attribute, $value, $fail) use ($userId) {
                    ...
                    $exists = UptimeMonitor::where('user_id', $userId)
                        ->whereRaw('LOWER(TRIM(TRAILING "/" FROM url)) = ?', [$url])
                        ->exists();
                    
                    if ($exists) {
                        $fail('This user already has an uptime monitor for this URL. Please edit the existing monitor instead.');
                    }
                },
            ],
            ...
            'expected_status_code_custom' => ['nullable', 'integer', 'min:100', 'max:599'],
        ...
        // Create SSL monitor if requested (same logic as client controller)
        ...
        // Create Domain monitor if requested (same logic as client controller)
        ...
        if ($request->has('create_domain_monitor') && $request->input('create_domain_monitor') == '1') {
            ...
            if ($domain) {
                ...
                $existingDomainMonitor = DomainMonitor::where('user_id', $userId)
                    ->where('domain', $domain)
                    ->first();
                
                if (!$existingDomainMonitor) {
                    $domainChannels = $validated['domain_communication_channels'] ?? ['email'];
        ...
        return redirect()->route('panel.uptime-monitors.index')
            ->with('success', $successMessage);
    }

    /**
     * Display the specified uptime monitor (admin can view any monitor).
     */
    public function show(Request $request, UptimeMonitor $uptimeMonitor): View
    {
        // Admin can view any monitor - no authorization check needed
        ...
        return view('panel.uptime-monitors.show', [
            'monitor' => $uptimeMonitor->load('user'),
    ...
    {
        // Admin can edit any monitor
        $users = \App\Models\User::where('role', 2)->orderBy('name')->get();
        
        return view('panel.uptime-monitors.edit', [
            'monitor' => $uptimeMonitor->load('user'),
            'users' => $users,
        ]);
    }
    ...
    {
        // Admin can update any monitor, but user_id remains locked to original owner
        $userId = $uptimeMonitor->user_id; // Keep original user_id, don't allow changes
        $user = \App\Models\User::findOrFail($userId);
        ...
        $validated = $request->validate([
            ...
            'url' => [
                ...
                'url', 
                'max:500',
                function ($attribute, $value, $fail) use ($userId, $uptimeMonitor) {
                    $url = rtrim(strtolower(trim($value)), '/');
                    $exists = UptimeMonitor::where('user_id', $userId)
                    ...
                    if ($exists) {
                        $fail('This user already has an uptime monitor for this URL. Please use a different URL or edit the existing monitor.');
                    }
                },
            ],
            'check_interval' => ['required', 'integer', 'in:1,3,5,10,30,60'],
        ...
        // Prepare update data (user_id remains unchanged - locked to original owner)
        $updateData = [
            'name' => $validated['name'],
            'url' => $validated['url'],
            'check_interval' => $validated['check_interval'],
        ...
        return redirect()->route('panel.uptime-monitors.show', $uptimeMonitor->uid)
    ...
    {
        // Admin can view any monitor's chart data
        $range = $request->input('range', '24h');
        $startDate = null;
        $endDate = null;
        $groupFormat = 'Y-m-d H:00';
    ...
    {
        // Admin can view any monitor's checks
    ...
    {
        // Admin can view any monitor's alerts
